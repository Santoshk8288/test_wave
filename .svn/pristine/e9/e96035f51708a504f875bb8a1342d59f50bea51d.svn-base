...
$basePath = "./services/server/uploads/queImage/";

 if($_SERVER['HTTP_HOST'] == "192.168.88.2"){
    $baseUrl = "testwave/services/server/uploads/queImage/";
}
else{
    $baseUrl = "services/server/uploads/queImage/";
}



//$basePath = "/opt/lampp/htdocs/testwave_editor/testwave/uploadQuestionImage/";
...
if (isset($_FILES['upload'])) {
    ...
    $name = strtotime("now").".".$file_ext;
	// It doesn't care if the file already exists, it's simply overwritten.
	$uploaded=move_uploaded_file($_FILES["upload"]["tmp_name"], $basePath . $name);
	if(!$uploaded)
	{
